// To display Rectangle -> Circle -> Triangle -> Incircle on Graph paper
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("RectCircTriIncircOnGraphPaper");
	int hCentre, vCentre;
	bool bDone = false;
	...
	// Create Window
	hwnd = CreateWindow(szAppName, TEXT("Rectangle->Circle->Triangle->Incircle on Graph paper"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, hCentre - (WIN_WIDTH / 2), vCentre - (WIN_HEIGHT / 2), WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
...
void Display() {
	// Variable declaration
	float i, radius = 1.5f, inRadius;
	const float rad = (22.0/7.0)/180.0;
	...
	for(i=-0.1; i>=-2.1f; i-=0.1) {
		glVertex3f(-2.0f, i, 0.0f);	// Points for horizontal line
		glVertex3f(2.0f, i, 0.0f);	// Points for horizontal line
		glVertex3f(i, -2.0f, 0.0f);	// Points for vertical line
		glVertex3f(i, 2.0f, 0.0f);	// Points for vertical line
	}
	glEnd();
	...
	glBegin(GL_LINES);			// Outermost rectangle
	...
	glPointSize(1);
	glBegin(GL_POINTS);			// Circle
	glColor3f(1.0f, 1.0f, 0.0f);
	for(i=0.0f; i<360.0f; i+=0.001f)
	glVertex3f((GLfloat)(radius*cos(i*rad)), (GLfloat)(radius*sin(i*rad)), 0.0f);
	glEnd();

	glBegin(GL_LINES);			// Inner triangle
	glColor3f(1.0f, 1.0f, 0.0f);
	glVertex3f(0.0f, radius, 0.0f);
	glVertex3f((GLfloat)(radius*cos(210*rad)), (GLfloat)(radius*sin(210*rad)), 0.0f);
	glVertex3f((GLfloat)(radius*cos(210*rad)), (GLfloat)(radius*sin(210*rad)), 0.0f);
	glVertex3f((GLfloat)(radius*cos(330*rad)), (GLfloat)(radius*sin(330*rad)), 0.0f);
	glVertex3f((GLfloat)(radius*cos(330*rad)), (GLfloat)(radius*sin(330*rad)), 0.0f);
	glVertex3f(0.0f, radius, 0.0f);
	glEnd();

	glPointSize(1);
	inRadius = radius / 2.0f;
	glBegin(GL_POINTS);			// InCircle
	glColor3f(1.0f, 1.0f, 0.0f);
	for(i=0.0f; i<360.0f; i+=0.001f)
		glVertex3f((GLfloat)(inRadius*cos(i*rad)), (GLfloat)(inRadius*sin(i*rad)), 0.0f);
	glEnd();
