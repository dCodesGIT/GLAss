// To display Circle on Graph paper with XY axis using GL_LINE_LOOP
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("CircleOnGraphPaper");
	int hCentre, vCentre;
	bool bDone = false;
	...
	// Create Window
	hwnd = CreateWindow(szAppName, TEXT("Circle on Graph paper using GL_LINE_LOOP"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, hCentre - (WIN_WIDTH / 2), vCentre - (WIN_HEIGHT / 2), WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
...
void Display() {
	...
	radius = 1.0f;
	pi = (22.0/7.0)/180.0;
	glBegin(GL_LINE_LOOP);			// hollow circle - using points
	glColor3f(1.0f, 1.0f, 0.0f);
	for(i=0.0f; i<360.0f; i+=0.01f) {
		x = radius * cos(i * pi);
		y = radius * sin(i * pi);
		glVertex3f((GLfloat)x, (GLfloat)y, 0.0f);
		glVertex3f((GLfloat)(radius*cos((i+0.01)*pi)), (GLfloat)(radius*sin((i+0.01)*pi)), 0.0f);
	}
	glEnd();
