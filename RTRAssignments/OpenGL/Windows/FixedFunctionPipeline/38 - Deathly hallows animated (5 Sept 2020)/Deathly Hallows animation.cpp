// To display Deathly hallows (Harry Potter) animation
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("DeathlyHallowsAnimation");
	int hCentre, vCentre;
	bool bDone = false;
	...
	// Create Window
	hwnd = CreateWindow(szAppName, TEXT("Deathly Hallows Animation"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, hCentre - (WIN_WIDTH / 2), vCentre - (WIN_HEIGHT / 2), WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
...
void Display() {
	// Variable declaration
	float i, radius, pi;
	static float translate = 0.025f, angle = 0.0f;
	static float stone = 2.0f, cloak = 2.0f, wand = 2.0f;
	...
	glTranslatef(0.0f, 0.0f, -5.0f);
	if(cloak > 0.0f)
		glTranslatef(cloak, -cloak, 0.0f);
	glRotatef(angle, 0.0f, 1.0f, 0.0f);
	glBegin(GL_LINES);		// Cloak of Invisiblity of Deathly hallows
	...
	glTranslatef(0.0f, 0.0f, -5.0f);
	if(wand > 0.0f)
		glTranslatef(0.0f, wand, 0.0f);
	glBegin(GL_LINES);		// The Elder Wand of Deathly hallows
	glVertex3f(0.0f, 1.0f, 0.0f);
	...
	cloak -= translate;
	wand -= translate;
	if(stone == 0.0f || cloak == 0.0f || wand == 0.0f)
		translate = 0.0f;
	if(angle <= 180.0f)
		angle += 1.0f;
