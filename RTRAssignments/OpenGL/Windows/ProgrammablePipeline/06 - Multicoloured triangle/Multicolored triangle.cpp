// Program to show Multicolored triangle
// By : Darshan Vikam
// Date : 15 March 2021
...
GLuint gVBObj_color;	// Fragment Buffer Object for color
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("MulticolouredTriangleInPP");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Multicoloured triangle"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
void Initialize(void) {
	...
	// For vColor
	glGenBuffers(1, &gVBObj_color);		// Generate buffer
	glBindBuffer(GL_ARRAY_BUFFER, gVBObj_color);	// Binding buffer
...
void Uninitialize(void) {
	...
	if(gVBObj_pos) {
		glDeleteVertexArrays(1, &gVBObj_pos);
		gVBObj_pos = 0;
	}
	if(gVBObj_color) {
		glDeleteVertexArrays(1, &gVBObj_color);
		gVBObj_color = 0;
	}
