...
GLuint gVBObj_color_Pyramid;	// Fragment Buffer Object for color
GLuint gVAObj_Cube;		// Vertex Array Object 
GLuint gVBObj_pos_Cube;		// Fragment Buffer Object for position
GLuint gVBObj_color_Cube;	// Fragment Buffer Object for color
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("3DShapesAnimationInPP");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("3D shapes animation"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
void Initialize(void) {
	...
	// vColor of Pyramid
	glGenBuffers(1, &gVBObj_color_Pyramid);		// Generate buffer
	glBindBuffer(GL_ARRAY_BUFFER, gVBObj_color_Pyramid);	// Binding buffer
	glBufferData(GL_ARRAY_BUFFER, sizeof(PyramidColors), PyramidColors, GL_STATIC_DRAW);	// Placing values in OpenGL's shader buffer(GL_VERTEX_BUFFER)
	...
	// vColor of Cube
	glGenBuffers(1, &gVBObj_color_Cube);		// Generate buffer
	glBindBuffer(GL_ARRAY_BUFFER, gVBObj_color_Cube);	// Binding buffer
	glBufferData(GL_ARRAY_BUFFER, sizeof(CubeColors), CubeColors, GL_STATIC_DRAW);	// Placing values in OpenGL's shader buffer(GL_VERTEX_BUFFER)
	glVertexAttribPointer(DV_ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 0, NULL);		// Formatting of data entered into OpenGL's shader buffer(GL_VERTEX_BUFFER)
	glEnableVertexAttribArray(DV_ATTRIBUTE_COLOR);	// Enabling of OpenGL's buffer(GL_VERTEX_BUFFER)
...
void Uninitialize(void) {
	...
	if(gVBObj_color_Pyramid) {
		glDeleteBuffers(1, &gVBObj_color_Pyramid);
		gVBObj_color_Pyramid = 0;
	}
	...
	if(gVBObj_color_Cube) {
		glDeleteBuffers(1, &gVBObj_color_Cube);
		gVBObj_color_Cube = 0;
	}
