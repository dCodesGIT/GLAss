// Program to draw triangle using geometry shader
// By : Darshan Vikam
// Date : 26 June 2021
...
GLuint gGSObj;		// Geometry Shader Object
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("GeometryShaderInPP");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Triangles using Geometry Shader"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
void Initialize(void) {
	...
	// Geometry Shader
	gGSObj = glCreateShader(GL_GEOMETRY_SHADER);		// Create shader
	const GLchar *GSSrcCode = 				// Source code of shader
		"#version 450 core \n" \
		"layout(triangles)in;" \
		...
		"void main(void) {" \
			...
			"}" \
		"}";
	glShaderSource(gGSObj, 1, (const GLchar**)&GSSrcCode, NULL);
	glCompileShader(gGSObj);				// Compile Shader
	ShaderErrorCheck(gGSObj, "COMPILE");
	...
	glAttachShader(gSPObj, gGSObj);		// Add Geometry shader code to final shader
...
void Uninitialize(void) {
	...
	glDetachShader(gSPObj, gGSObj);		// Detach geometry shader from final shader program
	...
	if(gGSObj != 0) {			// Delete Geometry shader
		glDeleteShader(gGSObj);
		gGSObj = 0;
	}
