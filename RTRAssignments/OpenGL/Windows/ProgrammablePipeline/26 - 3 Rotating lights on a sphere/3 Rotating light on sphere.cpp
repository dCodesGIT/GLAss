// Program to show 3 rotating lights on a Sphere with Per Vertex and Per Fragment lighting (toggling)
// By : Darshan Vikam
// Date : 08 April 2021
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("3RotatingLightsOnASphereInPP");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("3 Rotating lights on a Sphere"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
void Initialize(void) {
	...
	const GLchar *VSSrcCode_PVL = 				// Source code of shader
		...
		"uniform vec3 u_LAmb[3];" \
		"uniform vec3 u_LDiff[3];" \
		"uniform vec3 u_LSpec[3];" \
	...
	const GLchar *VSSrcCode_PFL = 				// Source code of shader
		...
		"void main(void) {" \
			"if(u_iKeyPressed == 1) {" \
				...
				"for(int i = 0; i < 3; i++) {" \
					"lightSource[i] = vec3(u_LPos[i] - eyeCoords);" \
				"}" \
			"}" \
			"gl_Position = u_PMatrix * u_VMatrix * u_MMatrix * vPosition;" \
		"}";
	...
	const GLchar *FSSrcCode_PFL = 			// Source code of shader
		...
		"void main(void) {" \
			...
			"if(u_iKeyPressed == 1) {" \
				"vec3 reflectionVector[3];" \
				"for(int i = 0; i < 3; i++) {" \
					"reflectionVector[i] = reflect(normalize(-lightSource[i]), normalize(transformedNormal));" \
					"vec3 ambient = u_LAmb[i] * u_KAmb;" \
					"vec3 diffuse = u_LDiff[i] * u_KDiff * max(dot(normalize(lightSource[i]), normalize(transformedNormal)), 0.0f);" \
					"vec3 specular = u_LSpec[i] * u_KSpec * pow(max(dot(reflectionVector[i], normalize(viewVector)), 0.0f), u_KShine);" \
...
void Display(void) {
	...
	if(gbEnableLighting == true) {
		...
		GLfloat lightPosition[] = 
			{ 0.0f, radius * (GLfloat)cos(xRotation * radian), radius * (GLfloat)sin(xRotation * radian), 1.0f,
			 radius * (GLfloat)sin(yRotation * radian), 0.0f, radius * (GLfloat)cos(yRotation * radian), 1.0f,
			 radius * (GLfloat)cos(zRotation * radian), radius * (GLfloat)sin(zRotation * radian), 0.0f, 1.0f };
		...
		glUniform3fv(gLightAmbientUniform, 3, lightAmbient);
		glUniform3fv(gLightDiffuseUniform, 3, lightDiffuse);
		glUniform3fv(gLightSpecularUniform, 3, lightSpecular);
		glUniform4fv(gLightPositionUniform, 3, lightPosition);
