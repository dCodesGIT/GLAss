// Program to show 2D animated shapes - triangle and square
// By : Darshan Vikam
// Date : 15 March 2021
...
GLuint gVBObj_color_square;	// Fragment Buffer Object for color
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("2DShapesAnimationInPP");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("2D shapes animation"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
void Initialize(void) {
	...
	// For vColor
	glGenBuffers(1, &gVBObj_color_square);		// Generate buffer
	glBindBuffer(GL_ARRAY_BUFFER, gVBObj_color_square);	// Binding buffer
	glBufferData(GL_ARRAY_BUFFER, sizeof(squareColors), squareColors, GL_STATIC_DRAW);	// Placing values in OpenGL's shader buffer(GL_VERTEX_BUFFER)
	glVertexAttribPointer(DV_ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 0, NULL);		// Formatting of data entered into OpenGL's shader buffer(GL_VERTEX_BUFFER)
	glEnableVertexAttribArray(DV_ATTRIBUTE_COLOR);	// Enabling of OpenGL's buffer(GL_VERTEX_BUFFER)
...
void Uninitialize(void) {
	...
	if(gVBObj_pos_triangle) {
		glDeleteVertexArrays(1, &gVBObj_pos_triangle);
		gVBObj_pos_triangle = 0;
	}
	if(gVBObj_color_triangle) {
		glDeleteVertexArrays(1, &gVBObj_color_triangle);
	...
	if(gVBObj_pos_square) {
		glDeleteVertexArrays(1, &gVBObj_pos_square);
		gVBObj_pos_square = 0;
	}
	if(gVBObj_color_square) {
		glDeleteVertexArrays(1, &gVBObj_color_square);
		gVBObj_color_square = 0;
	}
