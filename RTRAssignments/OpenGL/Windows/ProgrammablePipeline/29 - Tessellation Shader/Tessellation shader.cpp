// Program to show besier curve by using tessellation shader
// By : Darshan Vikam
// Date : 26 June 2021
...
GLuint gTCSObj;		// Tessellation Control Shader Object
GLuint gTESObj;		// Tessellation Evaluation Shader Object
...
GLuint gMVPUniform;	// Model View Matrix uniform
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("TessellationShaderInPP");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Besier Curve(Tessellation Shader)"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
void Initialize(void) {
	...
	ShaderErrorCheck(gTCSObj, "COMPILE");
	...
	ShaderErrorCheck(gTESObj, "COMPILE");
	...
	glAttachShader(gSPObj, gTCSObj);	// Add Tessellation Control shader to final shader
	glAttachShader(gSPObj, gTESObj);	// Add Tessellation Evaluation shader to final shader
	...
	gNumberOfSegmentsUniform = glGetUniformLocation(gSPObj, "numberOfSegments");
	gNumberOfStripsUniform = glGetUniformLocation(gSPObj, "numberOfStrips");
	gLineColorUniform = glGetUniformLocation(gSPObj, "lineColor");

	// other global variable initialization
	const GLfloat lineVertex[] = {
		-1.0f, -1.0f,
		-0.5f, 1.0f,
		0.5f, -1.0f,
		1.0f, 1.0f
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(lineVertex), lineVertex, GL_STATIC_DRAW);		// Placing values in OpenGL's shader buffer(GL_VERTEX_BUFFER)
	glVertexAttribPointer(DV_ATTRIBUTE_POSITION, 2, GL_FLOAT, GL_FALSE, 0, NULL);		// Formatting of data entered into OpenGL's shader buffer(GL_VERTEX_BUFFER)
...
void Display(void) {
	...
	glUniform1i(gNumberOfSegmentsUniform, uiNumberOfSegments);
	glUniform1i(gNumberOfStripsUniform, 1);
	glUniform4f(gLineColorUniform, 1.0f, 1.0f, 0.0f, 0.0f);
...
void Uninitialize(void) {
	...
	glDetachShader(gSPObj, gTCSObj);	// Detach tessellation control shader from final shader program
	glDetachShader(gSPObj, gTESObj);	// Detach tessellation evaluation shader from final shader program
	...
	if(gTCSObj != 0) {			// Delete Tessellation Control shader
		glDeleteShader(gTCSObj);
		gTCSObj = 0;
	}
	if(gTESObj != 0) {			// Delete Tessellation Evaluation shader
