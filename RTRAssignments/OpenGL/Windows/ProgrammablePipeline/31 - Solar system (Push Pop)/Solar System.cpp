// Program to show Solar system using push pop matrix
// By : Darshan Vikam
// Date : 15 March 2021
...
GLuint gVBObj_col_Sphere;	// Fragment Buffer Object for color 
...
void Initialize(void) {
	...
	gSphere_ptcnt = GenSphere(1.0f, 20, 20);
	GLfloat *sphere_Vertices = getSphereVertices();
	GLfloat *sphere_Color = setSphereColor(gSphere_ptcnt, 0.4f, 0.9f, 1.0f); 
	...
	glBindVertexArray(gVAObj_Sphere);
		...
		// vColor VBObj
		glGenBuffers(1, &gVBObj_col_Sphere);
		glBindBuffer(GL_ARRAY_BUFFER, gVBObj_col_Sphere);
		glBufferData(GL_ARRAY_BUFFER, gSphere_ptcnt * 2 * 3 * sizeof(GLfloat), sphere_Color, GL_STATIC_DRAW);
...
void Display(void) {
	...
	PushMatrix4x4(modelviewMatrix);
		...
		glUniform3f(gColorUniform, 1.0f, 1.0f, 0.0f);
		glBindVertexArray(gVAObj_Sphere);
	...
	PushMatrix4x4(modelviewMatrix);
		modelviewMatrix *= rotate((GLfloat)year, 0.0f, 1.0f, 0.0f);
		modelviewMatrix *= translate(4.0f, 0.0f, 0.0f);
		modelviewMatrix *= rotate((GLfloat)90.0f, 1.0f, 0.0f, 0.0f);
		modelviewMatrix *= rotate((GLfloat)day, 0.0f, 0.0f, 1.0f);
		modelviewMatrix *= scale(0.5f);
		modelviewProjectionMatrix = gPerspProjMatrix * modelviewMatrix;
		glUniformMatrix4fv(gMVPUniform, 1, GL_FALSE, modelviewProjectionMatrix);
		glUniform3f(gColorUniform, 0.4f, 0.9f, 1.0f);
		glBindVertexArray(gVAObj_Sphere);
			glDrawArrays(GL_TRIANGLES, 0, gSphere_ptcnt);
		glBindVertexArray(0);
	modelviewMatrix = PopMatrix4x4();
...
void Uninitialize(void) {
	...
	if(gVBObj_col_Sphere) {
		glDeleteVertexArrays(1, &gVBObj_col_Sphere);
		gVBObj_col_Sphere = 0;
	}
	deleteSphere();
