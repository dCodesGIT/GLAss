// Program to show 3D animated shapes - Pyramid and Cube
// By : Darshan Vikam
// Date : 15 March 2021
...
GLuint gVBObj_pos_Pyramid;	// Fragment Buffer Object for position
GLuint gVBObj_texture_Pyramid;	// Fragment Buffer Object for texture
GLuint gVAObj_Cube;		// Vertex Array Object 
GLuint gVBObj_pos_Cube;		// Fragment Buffer Object for position
GLuint gVBObj_texture_Cube;	// Fragment Buffer Object for texture
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("3DShapesWithTexturesInPP");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("3D shapes with Textures"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
void Initialize(void) {
	...
	glGenBuffers(1, &gVBObj_pos_Pyramid);		// Generate buffer
	glBindBuffer(GL_ARRAY_BUFFER, gVBObj_pos_Pyramid);	// Binding buffer
	...
	// vTexCoord of Pyramid
	glGenBuffers(1, &gVBObj_texture_Pyramid);		// Generate buffer
	glBindBuffer(GL_ARRAY_BUFFER, gVBObj_texture_Pyramid);	// Binding buffer
	glBufferData(GL_ARRAY_BUFFER, sizeof(PyramidTexture), PyramidTexture, GL_STATIC_DRAW);	// Placing values in OpenGL's shader buffer(GL_VERTEX_BUFFER)
	...
	// vColor of Cube
	glGenBuffers(1, &gVBObj_texture_Cube);		// Generate buffer
	glBindBuffer(GL_ARRAY_BUFFER, gVBObj_texture_Cube);	// Binding buffer
	glBufferData(GL_ARRAY_BUFFER, sizeof(CubeTexture), CubeTexture, GL_STATIC_DRAW);	// Placing values in OpenGL's shader buffer(GL_VERTEX_BUFFER)
	glVertexAttribPointer(DV_ATTRIBUTE_TEXTURE0, 2, GL_FLOAT, GL_FALSE, 0, NULL);		// Formatting of data entered into OpenGL's shader buffer(GL_VERTEX_BUFFER)
	glEnableVertexAttribArray(DV_ATTRIBUTE_TEXTURE0);	// Enabling of OpenGL's buffer(GL_VERTEX_BUFFER)
...
void Uninitialize(void) {
	...
	if(gVBObj_pos_Pyramid) {
		glDeleteBuffers(1, &gVBObj_pos_Pyramid);
		gVBObj_pos_Pyramid = 0;
	}
	if(gVBObj_texture_Pyramid) {
		glDeleteBuffers(1, &gVBObj_texture_Pyramid);
		gVBObj_texture_Pyramid = 0;
	}
	...
	if(gVBObj_texture_Cube) {
		glDeleteBuffers(1, &gVBObj_texture_Cube);
		gVBObj_texture_Cube = 0;
	}
