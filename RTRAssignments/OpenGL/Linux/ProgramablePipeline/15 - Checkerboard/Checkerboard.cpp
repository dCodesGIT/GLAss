// Checker Board in XWindows in Programmable Pipeline
// Date : 4 May 2021
// By : Darshan Vikam
...
GLuint gVAObj_Checkerboard;	// Vertex Array Object - Static smiley 
GLuint gVBObj_Checkerboard[2];	// Buffer Object - Static smiley = [1]-Vertices; [2]-TexCoord
...
void CreateWindow(void) {
	...
	XStoreName(gpDisplay, gWindow, "Checkerboard using procedural texture");
...
void Initialize(void) {
	...
	void loadProceduralTexture(void);	// Procedural Texture loading funtion
	...
	// For Quad
	glGenVertexArrays(1, &gVAObj_Checkerboard);
	glBindVertexArray(gVAObj_Checkerboard);		// For Quad
		glGenBuffers(2, gVBObj_Checkerboard);
		glBindBuffer(GL_ARRAY_BUFFER, gVBObj_Checkerboard[0]);		// For Position
		...
		glBindBuffer(GL_ARRAY_BUFFER, gVBObj_Checkerboard[1]);		// For Texture
...
void loadProceduralTexture(void) {
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, CheckerImageWidth, CheckerImageHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, (const void *)CheckerImage);
...
void Resize(int width, int height) {
	...
	gPerspMatrix = perspective(60.0f, (GLfloat)width/(GLfloat)height, 0.1f, 30.0f);
}
...
void display(void) {
	...
	// For straight quad
	glBindVertexArray(gVAObj_Checkerboard);
	glBindBuffer(GL_ARRAY_BUFFER, gVBObj_Checkerboard[0]);
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * 4 * 3, StraightCheckerBoard, GL_DYNAMIC_DRAW);
	glVertexAttribPointer(DV_ATTRIB_POS, 3, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(DV_ATTRIB_POS);
	...
	glBindVertexArray(gVAObj_Checkerboard);
	glBindBuffer(GL_ARRAY_BUFFER, gVBObj_Checkerboard[0]);
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * 4 * 3, TiltedCheckerBoard, GL_DYNAMIC_DRAW);
	glVertexAttribPointer(DV_ATTRIB_POS, 3, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(DV_ATTRIB_POS);
	glDrawArrays(GL_TRIANGLE_FAN, 0, 4);
	glBindVertexArray(0);
...
void Uninitialize() {
	...
	if(gVAObj_Checkerboard) {
		glDeleteVertexArrays(1, &gVAObj_Checkerboard);
		gVAObj_Checkerboard = 0;
	}

	// Destroy Vertex Buffer Object
	if(gVBObj_Checkerboard) {
		glDeleteBuffers(2, gVBObj_Checkerboard);
		gVBObj_Checkerboard[0] = 0;
		gVBObj_Checkerboard[1] = 0;
	}
