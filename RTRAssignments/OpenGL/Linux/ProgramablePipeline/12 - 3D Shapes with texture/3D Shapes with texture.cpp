...
void CreateWindow(void) {
	...
	XStoreName(gpDisplay, gWindow, "3D Shapes with textures");
...
void Initialize(void) {
	...
	glBindVertexArray(gVAObj_3D_Shapes[0]);		// For Pyramid
		...
		glBindBuffer(GL_ARRAY_BUFFER, gVBObj_Pyramid[1]);	// For Texture
	...
	glBindVertexArray(gVAObj_3D_Shapes[1]);		// For Cube
		...
		glBindBuffer(GL_ARRAY_BUFFER, gVBObj_Cube[1]);		// For Texture
	...
	Stone_texture = LoadBitmapAsTexture("stone.png");
	Kundali_texture = LoadBitmapAsTexture("vijay_kundali.png");
