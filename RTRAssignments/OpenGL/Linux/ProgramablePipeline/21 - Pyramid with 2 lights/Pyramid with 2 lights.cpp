// 2 Lights on rotating pyramid (Per Vertex) in XWindows in Programmable Pipeline
// Date : 6 May 2021
// By : Darshan Vikam
...
GLuint gVAObj_Shape;	// Vertex Array Object - 3D Sphere 
GLuint gVBObj_Pyramid[2];	// Buffer Object - Sphere[3] = [0]-Position; [1]-Normals;
...
void CreateWindow(void) {
	...
	XStoreName(gpDisplay, gWindow, "Pyramid with 2 lights");
...
void Initialize(void) {
	...
	const GLchar *VSSrcCode =			// Source code of shader
		...
		"void main(void) {" \
			"if(u_KeyPressed == 1) {" \
				...
				"vec3 ambient[2], diffuse[2], specular[2];" \
	...
	// For Pyramid
	glGenVertexArrays(1, &gVAObj_Shape);
	glBindVertexArray(gVAObj_Shape);		// For Sphere
		...
		glBindBuffer(GL_ARRAY_BUFFER, gVBObj_Pyramid[1]);	// For Normals
...
void display(void) {
	...
	glBindVertexArray(gVAObj_Shape);
...
void Uninitialize() {
	...
	if(gVAObj_Shape) {
		glDeleteVertexArrays(1, &gVAObj_Shape);
		gVAObj_Shape = 0;
	}
