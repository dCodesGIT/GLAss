// Checkerboard using procedure in WebGL
// By : Darshan Vikam
// Date : 08 July 2021
...
const checkerImageWidth = 64;
const checkerImageHeight = 64;
var checkerImage = new Array();
...
function initialize() {
	...
	gl.bindVertexArray(VAObj_quad);
		...
		gl.bufferData(gl.ARRAY_BUFFER, quadVertices.length, gl.DYNAMIC_DRAW);
	...
	checkerboard_texture = loadProceduralTexture(gl);
...
function loadProceduralTexture(gl) {
	// Code
	makeCheckerImage();
	...
	gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, checkerImageWidth, checkerImageHeight, 0, gl.RGB, gl.UNSIGNED_BYTE, checkerImage);
...
function makeCheckerImage() {
	// Code
	for(var i = 0; i < checkerImageHeight; i++) {
		for(var j = 0; j < checkerImageWidth; j++) {
			var c = (((i & 0x8) == 0) ^ ((j & 0x8) == 0)) * 255;
			checkerImage.push(c, c, c);
		}
	}
	checkerImage = Uint8Array.from(checkerImage);
}
...
function display() {
	// Variable declaration
	const StraightCheckerBoard = new Float32Array([
		// Straight board
	...
	const TiltedCheckerBoard = new Float32Array([
		// Tilted board
		1.0,  1.0, 0.0,		// Left top
		1.0, -1.0, 0.0,		// Left bottom
		2.41421, -1.0, -1.41421,// Right bottom
		2.41421,  1.0, -1.41421	// Right top
	]);
	...
	mat4.translate(TranslationMatrix, TranslationMatrix, [0.0, 0.0, -3.6]);
	...
	gl.bindVertexArray(VAObj_quad);
		gl.bindBuffer(gl.ARRAY_BUFFER, VBObj_quad[0]);
		gl.bufferData(gl.ARRAY_BUFFER, StraightCheckerBoard, gl.DYNAMIC_DRAW);
		gl.vertexAttribPointer(WebGLMacros.DV_ATTRIB_VERTEX, 3, gl.FLOAT, false, 0, 0);
		gl.enableVertexAttribArray(WebGLMacros.DV_ATTRIB_VERTEX);
	...
	gl.bindVertexArray(VAObj_quad);
		gl.bindBuffer(gl.ARRAY_BUFFER, VBObj_quad[0]);
		gl.bufferData(gl.ARRAY_BUFFER, TiltedCheckerBoard, gl.DYNAMIC_DRAW);
		gl.vertexAttribPointer(WebGLMacros.DV_ATTRIB_VERTEX, 3, gl.FLOAT, false, 0, 0);
		gl.enableVertexAttribArray(WebGLMacros.DV_ATTRIB_VERTEX);
		gl.drawArrays(gl.TRIANGLE_FAN, 0, 4);
	gl.bindVertexArray(null);
