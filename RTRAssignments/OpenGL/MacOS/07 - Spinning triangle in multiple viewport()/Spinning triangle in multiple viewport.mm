//
//  Spinning triangle in multiple viewport.mm
//  
...
-(void)applicationDidFinishLaunching : (NSNotification *)aNotification {
	...
	window = [[NSWindow alloc] initWithContentRect : win_rect
			...
			defer : NO];
	[window setTitle : @"Spinning triangle in multiple viewport"];
	[window center];
...
@implementation OpenGLView {
	...
	GLuint gVBObj[2];		// Fragment Buffer Object
...
-(void)reshape {
	...
	if(NSIsEmptyRect(myViewport))
		myViewport = (NSRect)[self bounds];
	
//	glViewport(0, 0, (GLsizei)rect.size.width, (GLsizei)rect.size.height);
//	gPerspProjMatrix = perspective(45.0f, (GLfloat)rect.size.width/(GLfloat)rect.size.height, 0.1f, 100.0f);

	glViewport(myViewport.origin.x, myViewport.origin.y, (GLsizei)myViewport.size.width, (GLsizei)myViewport.size.height);
	gPerspProjMatrix = perspective(45.0f, (GLfloat)myViewport.size.width/(GLfloat)myViewport.size.height, 0.1f, 100.0f);
...
-(void)keyDown : (NSEvent *)theEvent {
	...
	switch(key) {
		...
		case 'f' :
			...
			break;
		case 48 :		// Number 0
			myViewport = NSMakeRect(0, 0, width, height);
			[self reshape];
			[self drawView];
			break;
		case 49 :		// Number 1
			myViewport = NSMakeRect(0, 0, width/2, height/2);
			[self reshape];
			[self drawView];
			break;
		case 50 :		// Number 2
			myViewport = NSMakeRect(0, 0, width, height/2);
			[self reshape];
			[self drawView];
			break;
		case 51 :		// Number 3
			myViewport = NSMakeRect(width/2, 0, width/2, height/2);
			[self reshape];
			[self drawView];
			break;
		case 52 :		// Number 4
			myViewport = NSMakeRect(0, 0, width/2, height);
			[self reshape];
			[self drawView];
			break;
		case 53 :		// Number 5
			myViewport = NSMakeRect(width/4, height/4, width/2, height/2);
			[self reshape];
			[self drawView];
			break;
		case 54 :		// Number 6
			myViewport = NSMakeRect(width/2, 0, width/2, height);
			[self reshape];
			[self drawView];
			break;
		case 55 :		// Number 7
			myViewport = NSMakeRect(0, height/2, width/2, height/2);
			[self reshape];
			[self drawView];
			break;
		case 56 :		// Number 8
			myViewport = NSMakeRect(0, height/2, width, height/2);
			[self reshape];
			[self drawView];
			break;
		case 57 :		// Number 9
			myViewport = NSMakeRect(width/2, height/2, width/2, height/2);
			[self reshape];
			[self drawView];
			break;
	}
