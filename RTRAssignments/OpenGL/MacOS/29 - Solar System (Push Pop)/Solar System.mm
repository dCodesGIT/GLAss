...
-(void) drawView {		// Display() in windows
	...
	PushMatrix4x4(MVMatrix);
		MVMatrix *= rotate((GLfloat)year, 0.0f, 1.0f, 0.0f);
		MVMatrix *= translate(4.0f, 0.0f, 0.0f);
		MVMatrix *= rotate((GLfloat)90.0f, 1.0f, 0.0f, 0.0f);
		MVMatrix *= rotate((GLfloat)day, 0.0f, 0.0f, 1.0f);
		MVMatrix *= scale(0.5f);
		MVPMatrix = gPerspProjMatrix * MVMatrix;
		glUniformMatrix4fv(gMVPMatrixUniform, 1, GL_FALSE, MVPMatrix);
