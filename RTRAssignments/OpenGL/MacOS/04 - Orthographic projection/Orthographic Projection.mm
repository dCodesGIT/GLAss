//
//  Orthographic Projection.m
//  
//  Created by Darshan Vikam on 31/07/21.
//
...
-(void)applicationDidFinishLaunching : (NSNotification *)aNotification {
	...
	window = [[NSWindow alloc] initWithContentRect : win_rect
			...
			defer : NO];
	[window setTitle : @"Orthographic Projection"];
	[window center];
...
-(void)prepareOpenGL {
	...
	glBindAttribLocation(gSPObj, DV_ATTRIB_POS, "vPosition");	// Binding of shader program with enum declared
	...
	glBindVertexArray(gVAObj);		// Bind following steps into gVAObj variable
		...
		glVertexAttribPointer(DV_ATTRIB_POS, 3, GL_FLOAT, GL_FALSE, 0, NULL);		// Formatting of data entered into OpenGL's shader buffer(GL_VERTEX_BUFFER)
	...
	gOrthographicProjMatrix = mat4::identity();
...
-(void)reshape {
	...
	if(rect.size.width <= rect.size.height)
		gOrthographicProjMatrix = ortho(-100.0f, 100.0f, -100.0f*((GLfloat)rect.size.height/(GLfloat)rect.size.width), 100.0f*((GLfloat)rect.size.height/(GLfloat)rect.size.width), -100.0f, 100.0f);
	else
		gOrthographicProjMatrix = ortho(-100.0f*((GLfloat)rect.size.width/(GLfloat)rect.size.height), 100.0f*((GLfloat)rect.size.width/(GLfloat)rect.size.height), -100.0f, 100.0f, -100.0f, 100.0f);
...
-(void) drawView {		// Display() in windows
	...
	MVPMatrix = gOrthographicProjMatrix * MVMatrix;
