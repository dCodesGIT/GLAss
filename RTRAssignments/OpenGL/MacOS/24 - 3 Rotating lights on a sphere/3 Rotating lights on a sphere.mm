//
//  3 Rotating lights on a sphere.mm
//  
...
-(void)applicationDidFinishLaunching : (NSNotification *)aNotification {
	...
	window = [[NSWindow alloc] initWithContentRect : win_rect
			...
			defer : NO];
	[window setTitle : @"3 Rotating lights on a sphere"];
	[window center];
...
-(void)prepareOpenGL {
	...
	pointCnt = [sphere GenSphere : 1.0f stack : 30 slices : 60];
...
-(void) drawView {		// Display() in windows
	...
	*/	GLfloat lightSpecular[] = { 0.25f, 0.0f, 0.0f,
		...
		GLfloat lightPosition[] = {
			0.0f, radius * (GLfloat)cos(angle * rad), radius * (GLfloat)sin(angle * rad), 1.0f,
			radius * (GLfloat)sin(2 * angle * rad), 0.0f, radius * (GLfloat)cos(2 * angle * rad), 1.0f,
			radius * (GLfloat)cos(3 * angle * rad), radius * (GLfloat)sin(3 * angle * rad), 0.0f, 1.0f };
