package com.rtr3_android.geometry;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener {
	...
	private void initialize(GL10 gl) {
		...
		// Geometry Shader
		GSObj = GLES32.glCreateShader(GLES32.GL_GEOMETRY_SHADER);
		final String GSSrcCode = String.format(
			"#version 320 es \n"+
			"layout(triangles)in;"+
		...
		GLES32.glShaderSource(GSObj, GSSrcCode);
		GLES32.glCompileShader(GSObj);
		if(!ShaderErrorCheck(GSObj, GLES32.GL_GEOMETRY_SHADER)) {
			System.out.print("Error in Vertex Shader");
			return;
		}
		...
		GLES32.glAttachShader(SPObj, GSObj);
	...
	private boolean ShaderErrorCheck(int shaderObject, int type) {
		...
		if(type == GLES32.GL_VERTEX_SHADER || type == GLES32.GL_FRAGMENT_SHADER || type == GLES32.GL_GEOMETRY_SHADER)
	...
	private void Uninitialize() {
		...
		if(SPObj != 0) {
			...
			if(GSObj != 0) {
				GLES32.glDetachShader(SPObj, GSObj);
				GLES32.glDeleteShader(GSObj);
				GSObj = 0;
			}
			if(FSObj != 0) {
