package com.rtr3_android.threedwithtexture;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener {
	...
	private int[] VBObj_pyramid = new int[2];	// [0]-Position, [1]-Texture
	private int[] VBObj_cube = new int[2];		// [0]-Position, [1]-Texture
	...
	private void initialize(GL10 gl) {
		...
		GLES32.glBindVertexArray(VAObj_shapes[0]);		// For Pyramid
			...
			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, VBObj_pyramid[1]);		// TexCoords of pyramid
			ByteBuffer byteBuffer_t = ByteBuffer.allocateDirect(pyramidTexCoords.length * 4);
		...
		GLES32.glBindVertexArray(VAObj_shapes[1]);		// For cube
			...
			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, VBObj_cube[1]);		// TexCoords of cube
			byteBuffer_t = ByteBuffer.allocateDirect(cubeTexCoords.length * 4);
			byteBuffer_t.order(ByteOrder.nativeOrder());
			TexCoordBuffer = byteBuffer_t.asFloatBuffer();
	...
	private void draw() {
		...
		Matrix.translateM(translation_matrix, 0, -2.0f, 0.0f, -6.0f);
