// Orthographic projection in DirectX 3D
// By : Darshan Vikam
// Date : 7 August 2021
...
struct CBUFFER {
	XMMATRIX WorldViewProjectionMatrix;		// Similar to Model Matrix in OpenGL
};
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("OrthographicProjectionInDirect3D11");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Orthographic Projection in Direct3D 11"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
HRESULT Resize(int width, int height) {
	...
	if(width < height)
		gOrthographicProjectionMatrix = XMMatrixOrthographicOffCenterLH(-100.0f, 100.0f, -100.0f * ((float)height / (float)width), 100.0 * ((float)height / (float)width), -100.0f, 100.0f);
	else
		gOrthographicProjectionMatrix = XMMatrixOrthographicOffCenterLH(-100.0f * ((float)width / (float) height), 100.0f * ((float)width / (float) height), -100.0f, 100.0, -100.0f, 100.0f);
...
void Display(void) {
	...
	// translation is converned with world matrix transformation
