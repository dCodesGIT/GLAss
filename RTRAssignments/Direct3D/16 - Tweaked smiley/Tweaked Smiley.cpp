...
ID3D11Buffer* gpID3D11Buffer_VBO[2];
...
ID3D11ShaderResourceView* gpID3D11ShaderResourceView_Texture;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("TweakedSmileyInDirect3D11");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Tweaked Smiley in Direct3D 11"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
HRESULT Initialize(void) {
	...
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(quadVertices);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VBO[0], bufferDesc, quadVertices, sizeof(quadVertices));
	if(FAILED(hr))
		return hr;
	...
	ZeroMemory((void *)&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * 2 * 6;
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
/*		// Since data initialization is at runtime, we cannot use the function CreateBufferAndCopyData
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VBO[1], bufferDesc, (float *)NULL, sizeof(float) * 2 * 6);
	if(FAILED(hr))
		return hr;
*/	// We just create empty buffer for data to be filled
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VBO[1]);
	if(FAILED(hr)) {
	...
	if(FAILED(hr)) {
		WriteLog(FileName_log, "a+", "'smiley.bmp' texture loading failed !!!\n");
		return hr;
	}
	WriteLog(FileName_log, "a+", "'smiley.bmp' texture loaded successfully...\n");
...
void Display(void) {
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VBO[0], &stride, &offset);
	stride = sizeof(float) * 2;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VBO[1], &stride, &offset);
	...
	else if(keyPressed == 2) {		// 1/4 texture
		quadTexCoords[0] = 0.0f;
	...
	else if(keyPressed == 4) {		// Point texture
	...
	D3D11_MAPPED_SUBRESOURCE mappedSubresource;
	ZeroMemory((void *)&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VBO[1], 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, quadTexCoords, sizeof(quadTexCoords));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VBO[1], 0);

	// Load data into constant buffer
	CBUFFER constantBuffer_quad;
	constantBuffer_quad.WorldViewProjectionMatrix = wvpMatrix;
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer, 0, NULL, &constantBuffer_quad, 0, 0);
...
void Uninitialize(void) {
	...
	if(gpID3D11Buffer_VBO) {
		gpID3D11Buffer_VBO[0]->Release();
		gpID3D11Buffer_VBO[0] = NULL;
		gpID3D11Buffer_VBO[1]->Release();
		gpID3D11Buffer_VBO[1] = NULL;
	}
