...
ID3D11Buffer* gpID3D11Buffer_VBO_CloakOfInvisibility[2];	// Vertex Buffer Object for Cloak Of Invisibility
ID3D11Buffer* gpID3D11Buffer_VBO_ResurrectionStone[2];		// Vertex Buffer Object for Resurrection Stone
ID3D11Buffer* gpID3D11Buffer_VBO_ElderWand[2];			// Vertex Buffer Object for Elder Wand
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("DeathlyHallowsInDirect3D11");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Deathly Hallows in Direct3D 11"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
HRESULT Initialize(void) {
	...
	const float radians = XM_PI / 180.0f;
	const float triangleSide = 1.5f;
	const float triangle_radius = (2.0f / sqrt(3.0f)) * triangleSide;
	const float inCircle_radius = triangleSide / sqrt(3.0f);

	float CloakOfInvisibilityVertices[] =  {
		(float)(triangle_radius * cos(90.0f * radians)), (float)(triangle_radius * sin(90.0f * radians)), 0.0f,		// Apex
		(float)(triangle_radius * cos(210.0f * radians)), (float)(triangle_radius * sin(210.0f * radians)), 0.0f,	// Left bottom
		(float)(triangle_radius * cos(330.0f * radians)), (float)(triangle_radius * sin(330.0f * radians)), 0.0f,	// Right bottom
		(float)(triangle_radius * cos(90.0f * radians)), (float)(triangle_radius * sin(90.0f * radians)), 0.0f,		// Apex
	};
	float CloakOfInvisibilityColor[] = {
		1.0f, 0.0f, 0.0f,	// Apex
	...
	float ResurrectionStoneVertices[ptCnt_circle*3], ResurrectionStoneColor[ptCnt_circle*3];
	for(int i = 0; i < ptCnt_circle; i++) {
		float theta = (float)i * (360.0f / ptCnt_circle) * radians;
		ResurrectionStoneVertices[(i*3)+0] = inCircle_radius * cos(theta);
		ResurrectionStoneVertices[(i*3)+1] = inCircle_radius * sin(theta);
		ResurrectionStoneVertices[(i*3)+2] = 0.0f;
		ResurrectionStoneColor[(i*3)+0] = 1.0f;
		ResurrectionStoneColor[(i*3)+1] = 1.0f;
		ResurrectionStoneColor[(i*3)+2] = 0.0f;
	}

	float ElderWandVertices[] = {
		0.0f, (float)(inCircle_radius * 2), 0.0f,
		0.0f, (float)(-inCircle_radius), 0.0f,
	};
	float ElderWandColor[] = {
		1.0f, 0.0f, 0.0f,	// Red
	...
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(CloakOfInvisibilityVertices);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VBO_CloakOfInvisibility[0], bufferDesc, CloakOfInvisibilityVertices, sizeof(CloakOfInvisibilityVertices));
	if(FAILED(hr))
		return hr;
	...
	ZeroMemory((void *)&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(CloakOfInvisibilityColor);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VBO_CloakOfInvisibility[1], bufferDesc, CloakOfInvisibilityColor, sizeof(CloakOfInvisibilityColor));
	if(FAILED(hr))
		return hr;
	...
	ZeroMemory((void *)&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(ResurrectionStoneVertices);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VBO_ResurrectionStone[0], bufferDesc, ResurrectionStoneVertices, sizeof(ResurrectionStoneVertices));
	if(FAILED(hr))
		return hr;
	...
	ZeroMemory((void *)&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(ResurrectionStoneColor);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VBO_ResurrectionStone[1], bufferDesc, ResurrectionStoneColor, sizeof(ResurrectionStoneColor));
	if(FAILED(hr))
		return hr;
	...
	ZeroMemory((void *)&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(ElderWandVertices);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VBO_ElderWand[0], bufferDesc, ElderWandVertices, sizeof(ElderWandVertices));
	if(FAILED(hr))
		return hr;
	...
	ZeroMemory((void *)&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(ElderWandColor);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VBO_ElderWand[1], bufferDesc, ElderWandColor, sizeof(ElderWandColor));
	if(FAILED(hr))
		return hr;
...
void Display(void) {
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VBO_CloakOfInvisibility[0], &stride, &offset);
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VBO_CloakOfInvisibility[1], &stride, &offset);
	...
	rotationMatrix = XMMatrixRotationY(angle * XM_PI / 180.0f);
	worldMatrix *= rotationMatrix;
	translationMatrix = XMMatrixTranslation(-trans, -trans, 0.0f);
	worldMatrix *= translationMatrix;
	translationMatrix = XMMatrixTranslation(0.0f, -0.5f, 5.0f);
	worldMatrix *= translationMatrix;
	wvpMatrix = worldMatrix * viewMatrix * gPerspProjMatrix;

	// Load data into constant buffer
	CBUFFER constantBuffer_CloakOfInvisibility;
	constantBuffer_CloakOfInvisibility.WorldViewProjectionMatrix = wvpMatrix;
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer, 0, NULL, &constantBuffer_CloakOfInvisibility, 0, 0);
	...
	stride = sizeof(float) * 3;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VBO_ResurrectionStone[0], &stride, &offset);
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VBO_ResurrectionStone[1], &stride, &offset);
	...
	// Load data into constant buffer
	CBUFFER constantBuffer_ResurrectionStone;
	constantBuffer_ResurrectionStone.WorldViewProjectionMatrix = wvpMatrix;
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer, 0, NULL, &constantBuffer_ResurrectionStone, 0, 0);
	...
	stride = sizeof(float) * 3;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VBO_ElderWand[0], &stride, &offset);
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VBO_ElderWand[1], &stride, &offset);
	...
	viewMatrix = XMMatrixIdentity();
	translationMatrix = XMMatrixIdentity();
	translationMatrix = XMMatrixTranslation(0.0f, trans, 0.0f);
	worldMatrix *= translationMatrix;
	translationMatrix = XMMatrixTranslation(0.0f, -0.5f, 5.0f);
	worldMatrix *= translationMatrix;
	wvpMatrix = worldMatrix * viewMatrix * gPerspProjMatrix;
	...
	// Load data into constant buffer
	CBUFFER constantBuffer_ElderWand;
	constantBuffer_ElderWand.WorldViewProjectionMatrix = wvpMatrix;
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer, 0, NULL, &constantBuffer_ElderWand, 0, 0);
...
void Uninitialize(void) {
	...
	if(gpID3D11Buffer_VBO_CloakOfInvisibility) {
		gpID3D11Buffer_VBO_CloakOfInvisibility[0]->Release();
		gpID3D11Buffer_VBO_CloakOfInvisibility[0] = NULL;
		gpID3D11Buffer_VBO_CloakOfInvisibility[1]->Release();
		gpID3D11Buffer_VBO_CloakOfInvisibility[1] = NULL;
	}
	if(gpID3D11Buffer_VBO_ResurrectionStone) {
		gpID3D11Buffer_VBO_ResurrectionStone[0]->Release();
		gpID3D11Buffer_VBO_ResurrectionStone[0] = NULL;
		gpID3D11Buffer_VBO_ResurrectionStone[1]->Release();
		gpID3D11Buffer_VBO_ResurrectionStone[1] = NULL;
	}
	if(gpID3D11Buffer_VBO_ElderWand) {
		gpID3D11Buffer_VBO_ElderWand[0]->Release();
		gpID3D11Buffer_VBO_ElderWand[0] = NULL;
		gpID3D11Buffer_VBO_ElderWand[1]->Release();
		gpID3D11Buffer_VBO_ElderWand[1] = NULL;
	}
