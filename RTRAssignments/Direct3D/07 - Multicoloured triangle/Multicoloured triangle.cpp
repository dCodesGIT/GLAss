// Multicoloured triangle in DirectX 3D
// By : Darshan Vikam
// Date : 14 August 2021
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("MulticolouredTriangleInDirect3D11");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Multicoloured triangle in Direct3D 11"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
HRESULT Initialize(void) {
	...
	const char* VertexShaderSourceCode = 
		...
		"vertex_output main(float4 pos : POSITION, float4 col : COLOR) {" \
			...
			"return output;" \
		"}";
	void *result = CreateCompileAndSetShader(VertexShaderSourceCode, "VS", "vs_5_0", (void *)gpID3D11VertexShader);
	hr = (HRESULT)result;
	if(FAILED(hr))
		return hr;
	...
	const char* PixelShaderSourceCode = 
		...
		"float4 main(vertex_output input) : SV_TARGET {" \
			"return input.color;" \
		"}";
	result = CreateCompileAndSetShader(PixelShaderSourceCode, "PS", "ps_5_0", (void *)gpID3D11PixelShader);
	hr = (HRESULT)result;
	if(FAILED(hr))
		return hr;
	result = NULL;
	...
	const float triangleColors[] = {
		...
		0.0f, 0.0f, 1.0f,	// Right bottom - blue
...
/*	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VertexBuffer_Position, bufferDesc, triangleVertices, sizeof(triangleVertices));
	if(FAILED(hr))
		return hr;
*/
	...
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
/*	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VertexBuffer_Color, bufferDesc, triangleColors, sizeof(triangleColors));
	if(FAILED(hr))
		return hr;
*/
