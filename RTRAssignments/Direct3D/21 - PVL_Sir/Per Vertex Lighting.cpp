// Per Vertex Lighting in DirectX 3D
// By : Darshan Vikam
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("PerVertexLightingInDirect3D11");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Per Vertex Lighting in Direct3D 11"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
	...
	const char* VSSrcCode = 
		...
		"vertex_output main(float4 pos : POSITION, float4 norm : NORMAL) {" \
			...
			"if(lighting == 1) {" \
				...
				"float3 tNorm = (float3)normalize(mul((float3x3)mul(worldMatrix, viewMatrix), (float3)norm));" \
				"float3 lSrc =  (float3)normalize(lPos - eyeCoords);" \
				"float3 reflection_vector = reflect(-lSrc, tNorm);" \
				"float3 viewVector = normalize(-eyeCoords.xyz);" \
				"float4 ambient = lAmb * kAmb;" \
				"float4 diffuse = lDiff * kDiff * max(dot(lSrc, tNorm), 0.0);" \
				"float4 specular = lSpec * kSpec * pow(max(dot(reflection_vector, viewVector), 0.0), kShine);" \
