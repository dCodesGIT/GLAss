// Cube with lights in DirectX 3D
// By : Darshan Vikam
...
ID3D11Buffer* gpID3D11Buffer_VBO_Cube[2];			// Vertex Buffer Object for rectangle
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("CubeWithLightsInDirect3D11");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Cube with lights in Direct3D 11"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
HRESULT Initialize(void) {
	...
	const char* VSSrcCode = 
		...
		"struct vertex_output {" \
			...
			"float4 diff_light : COLOR;" \
		"};" \
		"vertex_output main(float4 pos : POSITION, float4 norm : NORMAL) {" \
			"vertex_output output;" \
			"output.diff_light = float4(1.0f, 0.0f, 0.0f, 1.0f);" \
			"if(lighting) {" \
				"float4 eyeCoords = mul(mul(viewMatrix, worldMatrix), pos);" \
				"float3 tNorm = normalize(mul((float3x3)mul(worldMatrix, viewMatrix), (float3)norm));" \
				"float3 lightSrc = (float3)normalize(lightPosition - eyeCoords);" \
				"output.diff_light = lightDiffuse * materialDiffuse * max(dot(lightSrc, tNorm), 0.0f);" \
			"}" \
	...
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VBO_Cube[1], bufferDesc, cubeNormals, sizeof(cubeNormals));
	if(FAILED(hr))
		return hr;
...
void Display(void) {
	...
	if(gbLightingEnabled) {
		constantBuffer_cube.lightEnabled = true;
		constantBuffer_cube.lDiff = XMVectorSet(1.0f, 1.0f, 1.0f, 1.0f);
		constantBuffer_cube.kDiff = XMVectorSet(0.5f, 0.5f, 0.5f, 1.0f);
		constantBuffer_cube.lPos = XMVectorSet(0.0f, 0.0f, -2.0f, 1.0f);
	}
	else
		constantBuffer_cube.lightEnabled = false;
	constantBuffer_cube.WorldMatrix = worldMatrix;
	constantBuffer_cube.ViewMatrix = viewMatrix;
	constantBuffer_cube.ProjectionMatrix = projectionMatrix;
