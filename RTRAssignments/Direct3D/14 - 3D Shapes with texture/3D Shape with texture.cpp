// 3D Shape (Pyramid and Cube) with texture in DirectX 3D
// By : Darshan Vikam
// Date : 22 August 2021
...
#include "WICTextureLoader.h"		// Windows Image Composer Texture Loader
...
ID3D11Buffer* gpID3D11Buffer_VBO_Pyramid[2];
ID3D11Buffer* gpID3D11Buffer_VBO_Cube[2];
...
ID3D11ShaderResourceView* gpID3D11ShaderResourceView_Texture[2];
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("3DShapeWithTexturesInDirect3D11");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("3D Shape with textures in Direct3D 11"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
HRESULT Initialize(void) {
	...
	HRESULT LoadD3DTexture(wchar_t const *, ID3D11ShaderResourceView **);
	HRESULT Resize(int, int);
	void Uninitialize(void);
	...
	const char* PSSrcCode = 
		...
		"float4 main(vertex_output input) : SV_TARGET {" \
			"float4 texel = tex2D.Sample(samplerState, input.texCoord);" \
			"return texel;" \
		"}";
	...
	inputElementDesc[1].AlignedByteOffset = D3D11_APPEND_ALIGNED_ELEMENT;		// Similar to glPixelStorei(alignment);
	...
	const float pyramidVertices[] = {
		// Front face - Apex, right bottom, left bottom
		0.0f, 2.0f, 0.0f,	1.0f, -1.0f, -1.0f,	-1.0f, -1.0f, -1.0f,
		
		// Right face - Apex, right bottom, left bottom
		0.0f, 2.0f, 0.0f,	1.0f, -1.0f, 1.0f,	1.0f, -1.0f, -1.0f,
		
		// Back face - Apex, right bottom, left bottom
		0.0f, 2.0f, 0.0f,	-1.0f, -1.0f, 1.0f,	1.0f, -1.0f, 1.0f,
		
		// Left face - Apex, right bottom, left bottom
		0.0f, 2.0f, 0.0f,	-1.0f, -1.0f, -1.0f,	-1.0f, -1.0f, 1.0f,
	};
	...
	const float cubeVertices[] = {
		// Front face - (Top left, Top right, Bottom right), (Bottom right, Bottom left, Top left)
		-1.0f, 1.0f, -1.0f,	1.0f, 1.0f, -1.0f,	1.0f, -1.0f, -1.0f,	1.0f, -1.0f, -1.0f,	-1.0f, -1.0f, -1.0f,	-1.0f, 1.0f, -1.0f,
		
		// Back face - (Top left, Top right, Bottom right), (Bottom right, Bottom left, Top left)
		1.0f, 1.0f, 1.0f,	-1.0f, 1.0f, 1.0f,	-1.0f, -1.0f, 1.0f,	-1.0f, -1.0f, 1.0f,	1.0f, -1.0f, 1.0f,	1.0f, 1.0f, 1.0f,
		
		// Right face - (Top left, Top right, Bottom right), (Bottom right, Bottom left, Top left)
		1.0f, 1.0f, -1.0f,	1.0f, 1.0f, 1.0f,	1.0f, -1.0f, 1.0f,	1.0f, -1.0f, 1.0f,	1.0f, -1.0f, -1.0f,	1.0f, 1.0f, -1.0f,
		
		// Left Face - (Top left, Top right, Bottom right), (Bottom right, Bottom left, Top left)
		-1.0f, 1.0f, 1.0f,	-1.0f, 1.0f, -1.0f,	-1.0f, -1.0f, -1.0f,	-1.0f, -1.0f, -1.0f,	-1.0f, -1.0f, 1.0f,	-1.0f, 1.0f, 1.0f,
		
		// Top face - (Top left, Top right, Bottom right), (Bottom right, Bottom left, Top left)
		-1.0f, 1.0f, 1.0f,	1.0f, 1.0f, 1.0f,	1.0f, 1.0f, -1.0f,	1.0f, 1.0f, -1.0f,	-1.0f, 1.0f, -1.0f,	-1.0f, 1.0f, 1.0f,
		
		// Bottom face - (Top left, Top right, Bottom right), (Bottom right, Bottom left, Top left)
		1.0f, -1.0f, 1.0f,	-1.0f, -1.0f, 1.0f,	-1.0f, -1.0f, -1.0f,	-1.0f, -1.0f, -1.0f,	1.0f, -1.0f, -1.0f,	1.0f, -1.0f, 1.0f,
	};
	...
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VBO_Pyramid[0], bufferDesc, pyramidVertices, sizeof(pyramidVertices));
	if(FAILED(hr))
		return hr;
	...
	ZeroMemory((void *)&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramidTexCoords);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VBO_Pyramid[1], bufferDesc, pyramidTexCoords, sizeof(pyramidTexCoords));
	if(FAILED(hr))
		return hr;
	...
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VBO_Cube[0], bufferDesc, cubeVertices, sizeof(cubeVertices));
	if(FAILED(hr))
		return hr;
	...
	ZeroMemory((void *)&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cubeTexCoords);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VBO_Cube[1], bufferDesc, cubeTexCoords, sizeof(cubeTexCoords));
	if(FAILED(hr))
		return hr;
	...
	hr = LoadD3DTexture(L"stone.bmp", &gpID3D11ShaderResourceView_Texture[0]);
	if(FAILED(hr)) {
		WriteLog(FileName_log, "a+", "'stone.bmp' texture loading failed !!!\n");
		return hr;
	}
	WriteLog(FileName_log, "a+", "'stone.bmp' texture loaded successfully...\n");

	hr = LoadD3DTexture(L"kundali.bmp", &gpID3D11ShaderResourceView_Texture[1]);
	if(FAILED(hr)) {
		WriteLog(FileName_log, "a+", "'kundali.bmp' texture loading failed !!!\n");
		return hr;
	}
	WriteLog(FileName_log, "a+", "'kundali.bmp' texture loaded successfully...\n");
	...
	if(FAILED(hr)) {
		WriteLog(FileName_log, "a+", "ID3D11Device::CreateSamplerState() failed  !!!\n");
		return hr;
	}
	WriteLog(FileName_log, "a+", "ID3D11Device::CreateSamplerState() succeeded...\n");
...
HRESULT LoadD3DTexture(wchar_t const *textureFilename, ID3D11ShaderResourceView **ppID3D11ShaderResourceView) {
	// Varible declaration
	HRESULT hr;
	...
	if(FAILED(hr))
		WriteLog(FileName_log, "a+", "CreateWICTextureFromFile() failed to load texture !!!");
	return hr;
}
...
void Display(void) {
	...
	UINT offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VBO_Pyramid[0], &stride, &offset);
	stride = sizeof(float) * 2;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VBO_Pyramid[1], &stride, &offset);
	...
	gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_Texture[0]);
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VBO_Cube[0], &stride, &offset);
	stride = sizeof(float) * 2;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VBO_Cube[1], &stride, &offset);
	...
	gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_Texture[1]);
	gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_Texture);
...
void Uninitialize(void) {
	...
	if(gpID3D11ShaderResourceView_Texture) {
		gpID3D11ShaderResourceView_Texture[1]->Release();
		gpID3D11ShaderResourceView_Texture[1] = NULL;
		gpID3D11ShaderResourceView_Texture[0]->Release();
		gpID3D11ShaderResourceView_Texture[0] = NULL;
	}
	...
	if(gpID3D11Buffer_VBO_Pyramid) {
		gpID3D11Buffer_VBO_Pyramid[0]->Release();
		gpID3D11Buffer_VBO_Pyramid[0] = NULL;
		gpID3D11Buffer_VBO_Pyramid[1]->Release();
		gpID3D11Buffer_VBO_Pyramid[1] = NULL;
	}
	if(gpID3D11Buffer_VBO_Cube) {
		gpID3D11Buffer_VBO_Cube[0]->Release();
		gpID3D11Buffer_VBO_Cube[0] = NULL;
		gpID3D11Buffer_VBO_Cube[1]->Release();
		gpID3D11Buffer_VBO_Cube[1] = NULL;
	}
