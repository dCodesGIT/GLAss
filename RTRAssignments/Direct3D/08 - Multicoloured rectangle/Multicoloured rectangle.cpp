// Multicoloured rectangle in DirectX 3D
// By : Darshan Vikam
// Date : 14 August 2021
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("MulticolouredRectangleInDirect3D11");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Multicoloured rectangle in Direct3D 11"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
HRESULT Initialize(void) {
	...
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VertexBuffer_Position, bufferDesc, rectangleVertices, sizeof(rectangleVertices));
	if(FAILED(hr))
		return hr;
	...
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = CreateBufferAndCopyData(&gpID3D11Buffer_VertexBuffer_Color, bufferDesc, rectangleColors, sizeof(rectangleColors));
	if(FAILED(hr))
		return hr;
