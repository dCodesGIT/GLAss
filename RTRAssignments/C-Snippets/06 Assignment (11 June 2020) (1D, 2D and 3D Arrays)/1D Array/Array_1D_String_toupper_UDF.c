// To demonstrate 1D array - String operation to capitalize all characters - User defined function
// Date : 11 June 2020
// By : Darshan Vikam
...
int main() {
	...
	for(i = 0; i < length; i++) {
		if(i == 0)
			str_capital[i] = capitalize(str_original[i]);
		else if(str_original[i] == ' ') {
			str_capital[i] = str_original[i];
			i++;
			if(!(str_original[i] >= 'a' && str_original[i] <= 'z')) {
				str_capital[i] = str_original[i];
				i++;
			}
			str_capital[i] = capitalize(str_original[i]);
